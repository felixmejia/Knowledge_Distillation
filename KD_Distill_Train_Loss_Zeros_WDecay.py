...
def benchmark_inference_on_validation(
    model, dataset, device, model_name,
    sizes=(1000, 5000, 10000, 20000, 30000, 40000),
    batch_size=128, num_workers=4,
    output_dir='benchmarks',
    assumed_power_watts=300.0,         # si no hay CodeCarbon
    emissions_factor_kg_per_kwh=0.5    # si no hay CodeCarbon (promedio global aprox)
):
    """
    Corre inferencia en subconjuntos de validación de tamaños dados y mide
    tiempo, energía (kWh) y CO₂eq (kg). Guarda resultados en CSV.

    - Si CodeCarbon está disponible: usa medición real.
    - Si no: estima energía = (potencia asumida * tiempo) / 3.6e6 [kWh]
            y CO2 = energía * factor_emisión.

    Crea un CSV por modelo en `output_dir`:
      {output_dir}/{model_name}_validation_inference_benchmark.csv
    """
    ...
    for N in sizes:
        ...
        # Iniciar medición de energía si hay CodeCarbon
        emissions_kgco2eq = 0.0
        energy_kwh = 0.0
        tracker = None
        ...
        if CODECARBON_AVAILABLE:
            emissions_csv_path = os.path.join(
                output_dir, f"{model_name}_inference_{n_eff}.csv"
            )
        ...
        if CODECARBON_AVAILABLE and tracker is not None:
            ...
            # Leer energy_consumed (kWh) del CSV de CodeCarbon
            try:
                if os.path.exists(emissions_csv_path):
                    df_cc = pd.read_csv(emissions_csv_path)
                    energy_kwh = float(df_cc.iloc[-1]['energy_consumed'])
            except Exception as e:
                print(f"[WARN] No se pudo leer energía de CodeCarbon: {e}")
                energy_kwh = 0.0
        else:
            # Estimación sin CodeCarbon
        ...
        print(f"[{model_name}] N={n_eff} | {elapsed_s:.2f}s | "
              f"{throughput:.2f} img/s | {energy_kwh:.6f} kWh | "
              f"{emissions_kgco2eq:.6f} kgCO₂eq")

    df = pd.DataFrame(results)
    out_csv = os.path.join(output_dir, f"{model_name}_validation_inference_benchmark.csv")
    df.to_csv(out_csv, index=False)
    logging.info(f"Saved inference benchmark: {out_csv}")
    print(f"Saved inference benchmark: {out_csv}")
...
def main():
    ...
    if args.resume:
        ...
        student_model, student_optimizer, student_scheduler, student_start_epoch, student_history, student_thresholds = load_checkpoint(
            student_model, student_optimizer, student_scheduler, distilled_checkpoint, device
        )
        ...
        summary_table, _ = evaluate_models(
            ...
            ['Teacher', 'Student'], val_loader, device, plot_dir,
            thresholds=student_thresholds
        )
